import qiskit as q
import numpy as np
import math as math
import scipy as sc

statevector_simulator = q.Aer.backends(name='statevector_simulator')[0]
qasm_simulator = q.Aer.backends(name='qasm_simulator')[0]

#==========================================
#----------     Wavefunction     ----------
#==========================================

def Wavefunction(obj, *args, **kwargs):
    '''
    Displays the wavefunction of a quantum system
    '''
    if(type(obj) == q.QuantumCircuit):
        statevec = q.execute(obj, statevector_simulator,shots=1).result().get_statevector()
    if(type(obj) == np.ndarray):
        statevec = obj
    sys = False
    NL = False
    dec = 5
    if 'precision' in kwargs:
        dec = int(kwargs['precision'])
    if 'column' in kwargs:
        NL = kwargs['column']
    if 'systems' in kwargs:
        systems = kwargs['systems']
        sys = True
        last_sys = int(len(systems)-1)
        show_systems = []
        for s_chk in np.arange(len(systems)):
            if(type(systems[s_chk])!=int):
                raise Exception('systems must be an array of all integers!')
        if 'show_systems' in kwargs:
            show_systems = kwargs['show_systems']
            if(len(systems)!=len(show_systems)):
                raise Exception('systems and show_systems need to be arrays of equal length!')
            for ls in np.arange(len(show_systems)):
                if((show_systems[ls]!=True)and(show_systems[ls]!=False)):
                    raise Exception('show_systems must be an array of Truth Values!')
                if(show_systems[ls]==True):
                    last_sys=int(ls)
        else:
            for ss in np.arange(len(systems)):
                show_systems.append(True)
    wavefunction = ''
    qubits = int(m.log(len(statevec),2))
    for i in np.arange(int(len(statevec))):
        value = round(statevec[i].real, dec)+round(statevec[i].imag, dec)*1j
        if ((value.real!=0) or (value.imag!=0)):
            state=list(Binary2(int(i),int(2**qubits)))
            state_str = ''
            if (sys == True): #Systems and Show_Systems
                k=0
                for s in np.arange(len(systems)):
                    if(show_systems[s]==True):
                        if(int(s)!=last_sys):
                            state.insert(int(k+systems[s]),'>|')
                            k = int(k+systems[s]+1)
                        else:
                            k = int(k+systems[s])
                    else:
                        for s2 in np.arange(systems[s]):
                            del state[int(k)]
            for j in np.arange(len(state)):
                if(type(state[j])!=str):
                    state_str=state_str+str(int(state[j]))
                else:
                    state_str=state_str+state[j]
                if((value.real!=0) and (value.imag!=0)):
                    if(value.imag > 0):
                        wavefunction = wavefunction + str(value.real)+'+'+str(value.imag)+'j |'+state_str+'>   '
                    else:
                        wavefunction = wavefunction + str(value.real)+''+str(value.imag)+'j |'+state_str+'>   '
                if((value.real!=0) and (value.imag==0)):
                    wavefunction = wavefunction + str(value.real)+' |'+state_str+'>   '
                if((value.real==0) and (value.imag!=0)):
                    wavefunction = wavefunction + str(value.imag)+'j |'+state_str+'>   '
                if(NL):
                    wavefunction = wavefunction + '\n'
    print(wavefunction)

def Measurement(quantumcircuit, *args, **kwargs):
    '''
    Displays the measurement results of a quantum circuit
    '''
    p_M = True
    S=1
    ret = False
    NL = False
    if 'shots' in kwargs:
        S = int(kwargs['shots'])
    if 'return_M' in kwargs:
        ret = kwargs['return_M']
    if 'print_M' in kwargs:
        p_M = kwargs['print_M']
    if 'column' in kwargs:
        NL = kwargs['column']

    M1 = q.execute(quantumcircuit, qasm_simulator, shots=S).result().get_counts(quantumcircuit)
    M2 = {}
    k1 = list(M1.keys())
    v1 = list(M1.values())

    for k in np.arange(len(k1)):
        key_list = list(k1[k])
        new_key = ''
        for j in np.arange(len(key_list)):
            new_key = new_key+key_list[len(key_list)-(j+1)]
        M2[new_key] = v1[k]
    if(p_M):
        k2 = list(M2.keys())
        v2 = list(M2.values())
        measurements = ''
        for i in np.arange(len(k2)):
            m_str = str(v2[i])+'|'
            for j in np.arange(len(k2[i])):
                if(k2[i][j] == '0'):
                    m_str = m_str+'0'
                if(k2[i][j] == '1'):
                    m_str = m_str+'1'
                if(k2[i][j] == ' '):
                    m_str = m_str+'>|'
            m_str = m_str+'>   '
            if(NL):
                m_str = m_str + '\n'
            measurements = measurements + m_str
        print(measurements)
    if(ret):
        return M2

#==================================
#----------     Math     ----------
#==================================

def Oplus(bit1,bit2):
    '''
    Adds two bits of 0's and 1's (modulo 2)
    '''
    bit = np.zeros(len(bit1))
    for i in np.arange(len(bit)):
        if((bit1[i]+bit2[i])%2==0):
            bit[i] = 0
        else:
            bit[i] = 1
    return bit

def Binary(number,total):
    '''
    Converts a number to binary, right to left <-- LSB
    '''
    qubits = int(math.log(total,2))
    N = number
    b_num = np.zeros(qubits)
    for i in np.arange(qubits):
        if(N/((2)**(qubits-i-1))>=1):
            b_num[i] = 1
        # Missing
        # Missing
    for j in np.arange(len(b_num)):
        B.append(int(b_num[j]))
    return B

def From_Binary(string):
    '''
    Converts a binary number to base 10m right to left LSB
    '''
    num = 0
    for i in np.arange(len(string)):
        num = num + string[int(0-(i+1))*2**(i)]
    return num

def X_Transformation(qc, qreg, state):
    '''
    Transforms the state of the system, applying X gates according to 0's in the vector state
    '''
    for j in np.arange(len(state)):
        if(int(state[j])==0):
            qc.x(qreg[int(j)])

def Grover_Oracle(mark, qc, q, an1, an2):
    '''
    Picks out the marked state and applies a negative phase
    '''
    qc.h(an1[0])
    X_Transformation(qc, q, mark)
    if(len(mark)>2):
        n_NOT(qc, q, an1[0], an2)
    if(len(mark)==2):
        qc.ccx(q[0], q[1], an1[0])
    X_Transformation(qc, q, mark)
    qc.h(an1[0])

def Grover_Diffusion(mark, qc, q, an1, an2):
    '''
    Amends the instructions for a Grover Diffusion Operation to the QuantumCircuit
    '''
    zeros_state = []
    for i in np.arange(len(mark)):
        zeros_state.append(0)
        qc.h(q[int(i)])
    Grover_Oracle(zeros_state, qc, q, an1, an2)
    for j in np.arange(len(mark)):
        qc.h(q[int(j)])

def Grover(Q, marked):
    '''
    Amends the instructions for a Grover Search
    '''
    q    = q.QuantumRegister(Q,   name='q'   )
    anc1 = q.QuantumRegister(1,   name='anc' )
    anc2 = q.QuantumRegister(Q-2, name='nanc')
    c    = q.ClassicalRegister(Q, name='c'   )
    qc   = q.QuantumCircuit(q, anc1, anc2, c, name='qc')

    for j in np.arange(Q):
        qc.h(q[int(j)])
    qc.x(anc1[0])
    iterations = round(math.pi/4 * 2 ** (Q/2.0))
    for i in np.arange(iterations):
        Grover_Oracle(marked, qc, q, anc1, anc2)
        Grover_Diffusion(marked, qc, q, anc1, anc2)
    return qc, q, anc1, anc2, c


